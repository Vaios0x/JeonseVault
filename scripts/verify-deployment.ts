  ...
  try {
    ...
    for (const [contractName, contractInfo] of Object.entries(deployment.contracts)) {
      ...
      try {
        ...
        console.log(`   ⚠️ No se pudo verificar ${contractName}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    if (allRolesCorrect) {
      ...
      console.log('   Ejecuta: npx hardhat run scripts/transfer-ownership.ts --network kairos');
    }
